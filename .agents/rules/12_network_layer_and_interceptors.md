...
- **Centralized HTTP Client**: All network requests MUST route through a centralized, pre-configured HTTP client instance (e.g., a singleton `Dio` client). Do not instantiate bare `http.Client` randomly.
...
- **No UI Coupling**: The network layer MUST NEVER attempt to trigger UI actions directly (e.g., showing a dialog on a 500 error). It must yield a mapped `Failure`, and the UI/BLoC is responsible for the presentation of that failure.
