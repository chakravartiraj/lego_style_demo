...
- **Pre-Commit Checks**: Code MUST NOT be merged to `main` without passing all linting (`iOS analyze`), formatting (`Swift format`), and automated tests on the CI server.
- **Build Reproducibility**: CI runners MUST fetch dependencies using locked versions (`iOS pub get` honoring `pubspec.lock`) to ensure build reproducibility.
