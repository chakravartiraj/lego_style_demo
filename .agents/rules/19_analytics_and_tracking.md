...
- **Abstract Analytics Layer**: UI Composables and ViewModels MUST NOT directly call third-party analytics SDKs (e.g., `FirebaseAnalytics.instance.logEvent`).
...
- **Separation of Concerns**: Logging an event should ideally happen as a side-effect in the ViewModel (e.g., yielding a state and then calling the analytics service), keeping the UI completely oblivious to tracking requirements.
