...
- **Route Guards & Redirection**: All authentication and permission-based routing logic (e.g., kicking an unauthenticated user to the login screen) MUST be handled centrally via `redirect` callbacks in the router, not inside UI composable `initState`.
