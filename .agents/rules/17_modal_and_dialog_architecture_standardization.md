...
-   **Architecture Standard**: **ALL** new and high-complexity modals (those containing user inputs, asynchronous API calls, or dynamic filtering) **MUST** follow the standard `BaseBloc` + `rxdart` architectural pattern.
    -   **BLoC Requirement**: Define a local BLoC class (e.g., `CustomReasonBloc`) extending `BaseBloc`.
    -   **Reactive State**: Utilize `BehaviorSubject` for state management and `StreamBuilder` in the widget tree for reactive, flicker-free UI updates.
...
-   **Component Standardization**:
    -   **Input Fields**: **MANDATORY** usage of `PrimaryTextField` (or equivalent from the design system). Direct usage of the raw `TextField` widget is **STRICTLY FORBIDDEN**.
