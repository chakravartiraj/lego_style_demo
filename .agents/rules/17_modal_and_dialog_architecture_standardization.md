...
-   **Architecture Standard**: **ALL** new and high-complexity modals (those containing user inputs, asynchronous API calls, or dynamic filtering) **MUST** follow the standard `BaseViewModel` + `rxkotlin` architectural pattern.
    -   **ViewModel Requirement**: Define a local ViewModel class (e.g., `CustomReasonViewModel`) extending `BaseViewModel`.
    -   **Reactive State**: Utilize `BehaviorSubject` for state management and `StreamBuilder` in the composable tree for reactive, flicker-free UI updates.
...
-   **Component Standardization**:
    -   **Input Fields**: **MANDATORY** usage of `PrimaryTextField` (or equivalent from the design system). Direct usage of the raw `TextField` composable is **STRICTLY FORBIDDEN**.
