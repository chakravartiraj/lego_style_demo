...
- **Sociable Unit Tests**: Rather than writing highly mocked, isolated unit tests for every single class, prefer "sociable" tests. Test the ViewModel, the UseCase (if any), and the Repository together in a single test suite.
...
- **State Verification**: A sociable test should arrange the network mock to return a specific JSON response, dispatch a ViewModel event, and verify the resulting ViewModel states (loading -> loaded with parsed domain entities).
