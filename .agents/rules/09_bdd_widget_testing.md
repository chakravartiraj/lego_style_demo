# 09. BDD Widget Testing (Behavior-Driven Development)
...
- **Code Generation (`bdd_widget_test`)**: Use the `bdd_widget_test` package to automatically generate Flutter widget tests from the `.feature` files.
...
- **Mocking at the Edge**: In BDD tests, pump the full feature widget and inject mocked repositories or a mocked network client to simulate the backend.
