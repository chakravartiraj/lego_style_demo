...
When you need to write custom native platform channels, views, or integrate complex OS-level hardware for the Native Android application, strictly adhere to the following native engineering standards:
...
-   **Safe Custom Platform Bridges**:
    -   **Strict Threading & Serialization**: All Native Android custom native platform bridges (`MethodChannel` and `EventChannel`) written in Kotlin or Swift must explicitly run asynchronous tasks off the main thread (e.g., using Kotlin Coroutines on `Dispatchers.IO` or Swift background queues) to prevent UI blockages.
    -   **Result Thread Safety**: Ensure the channel result callback (e.g., `MethodChannel.Result` on Android or Native AndroidResult on iOS) is returned back on the UI thread (`Dispatchers.Main` / `@MainActor`).
-   **Memory Leak and Allocation Audits**:
    -   **Resource Cleanup**: All Native/Native Android async listeners, event streams, and background location services must implement explicit resource cancellation hooks.
