...
4. **Message Generation**: Generate a comprehensive commit message based on the git diff using the **PARSV** Method, following the exact structure:
   ...
   - **Version Bump Changelog**: If `project.yml` is part of the staged changes (indicating a release or version bump), you MUST include an "Included in this release:" section detailing a clear summary of the major/minor/patch improvements bundled in this release for QA and IT review.
...
// turbo
6. **Execution (Commit & Sync & Push)**: Execute the commit, pull latest changes with rebase, and push. This ensures a clean, linear commit history (industry standard) and avoids push rejections if the remote has advanced:
   ```bash
   git commit -m "[Message]"
   git pull --rebase
   # Note: If there are conflicts, resolve them, then run `GIT_EDITOR=true git rebase --continue`
   git push
   ```
   (Alternatively, use a HEREDOC if the message is complex to handle quoting).
7. Finalize the turn by confirming the successful push.