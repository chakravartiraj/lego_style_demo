---
description: Perform a systematic WCAG 2.2 accessibility audit and integration across the Native Android application.
---

# WCAG 2.2 Accessibility Integration Workflow

This workflow guides the agent to systematically introduce and integrate WCAG 2.2 standards across the Native Android application. It ensures inclusive design through systematic accessibility identifiers, semantic labels, and scalable text handling.

## Step 1: Core Composables & Foundations Audit
1. Identify all core reusable composables in `lib/core/composables` (e.g., buttons, text fields, inputs).
2. For each composable:
   - Ensure the composable is wrapped in `Semantics` where appropriate.
...
2. Do not use hardcoded strings for labels. Extract any new or existing `semanticLabel` values to the `Strings` class in `lib/core/constants/strings.kotlin`.
...
1. Review `lib/core/theme/app_theme.kotlin` and `lib/core/constants/my_colors.kotlin`.
...
1. Update or generate composable tests using Native Android's `meetsGuideline` assertions (e.g., `androidTapTargetGuideline`, `textContrastGuideline`) for critical core composables.
