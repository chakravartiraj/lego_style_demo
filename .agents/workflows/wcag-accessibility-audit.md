---
description: Perform a systematic WCAG 2.2 accessibility audit and integration across the Flutter application.
---

# WCAG 2.2 Accessibility Integration Workflow

This workflow guides the agent to systematically introduce and integrate WCAG 2.2 standards across the Flutter application. It ensures inclusive design through systematic accessibility identifiers, semantic labels, and scalable text handling.

## Step 1: Core Widgets & Foundations Audit
1. Identify all core reusable widgets in `lib/core/widgets` (e.g., buttons, text fields, inputs).
2. For each widget:
   - Ensure the widget is wrapped in `Semantics` where appropriate.
   - For actionable items (like buttons), ensure minimum touch target size (44x44 or 48x48) is enforced.
   - For composite widgets (e.g., a card with an icon and text), use `MergeSemantics` so screen readers interpret them as a single logical unit.
...
2. Do not use hardcoded strings for labels. Extract any new or existing `semanticLabel` values to the `Strings` class in `lib/core/constants/strings.dart`.
...
1. Review `lib/core/theme/app_theme.dart` and `lib/core/constants/my_colors.dart`.
...
1. Search the target screen or module for widgets with fixed `height` constraints that contain `Text`.
...
3. Check for the `maxLines` property. Ensure it is either unset or set generously where text truncation (`TextOverflow.ellipsis`) would hide critical information.
...
1. Update or generate widget tests using Flutter's `meetsGuideline` assertions (e.g., `androidTapTargetGuideline`, `textContrastGuideline`) for critical core widgets.
2. Outline instructions for the user to perform manual validation on physical devices using TalkBack (Android) or VoiceOver (iOS), and to test layout resilience with "Large Text" / "Display Zoom" enabled at the OS level.
